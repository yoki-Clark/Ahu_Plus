        ...
        val loc = withTimeoutOrNull(timeoutMs) {
            suspendCancellableCoroutine<Location?> { cont ->
                ...
                if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.R) {
                    // API 30+:用非废弃的 getCurrentLocation(单次异步取点,系统自动超时)
                    ...
                    cont.invokeOnCancellation { runCatching { cancellationSignal.cancel() } }
                } else {
                    // API 24-25:无 getCurrentLocation,只能用已废弃的 requestSingleUpdate
