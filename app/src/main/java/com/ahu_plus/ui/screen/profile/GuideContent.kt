    ...
    GuideCategory(
        ...
        entries = listOf(
            ...
            GuideEntry(
                ...
                sections = listOf(
                    usage {
                        p("应用中心按学习、通知、校园卡、生活和个人信息分组，功能入口以双栏卡片展示，也支持按标题和分组搜索。已启用的集市、学习通和 WeLearn 会显示为第三方服务入口。")
    ...
    GuideCategory(
        ...
        entries = listOf(
            ...
            GuideEntry(
                ...
                sections = listOf(
                    usage {
                        p("考试页展示教务系统返回的课程、时间、考场和座位等信息，已结束考试会弱化显示。页面支持写入系统日历。")
                    },
                    impl {
    ...
    GuideCategory(
        ...
        entries = listOf(
            GuideEntry(
                ...
                sections = listOf(
                    usage {
                        p("校园卡区域提供实时余额、消费账单、消费分析和智慧安大支付码。支付码可放大全屏，并可按设置临时调高屏幕亮度；打开后会在页面可见期间自动刷新，连续失败时会降低刷新频率。")
            ...
            GuideEntry(
                ...
                sections = listOf(
                    usage {
                        p("消费账单展示一卡通交易记录；消费分析支持按月和按学期汇总总额、日均、趋势、分类、商户排行和餐次分布，学期起止日期来自教务系统。")
                    },
                    impl {
            ...
            GuideEntry(
                ...
                sections = listOf(
                    usage {
                        ...
                        p("视频从服务端记录位置继续，倍速可配置为 0.1x 到 1.0x；文档、阅读、音频和直播任务会按服务端结果更新。可选的自动签到与访问次数更新在学习结束后执行。")
                        p("课程、章节和任务当前串行执行，并发章节数仅保留兼容配置。所有请求仍经过账号/主机级流量治理；遇到 403、429、验证码或限制页时会停止当前批次，不会连续重试。")
