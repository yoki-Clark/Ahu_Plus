...
val faqCategories: List<FaqCategory> = listOf(
    ...
    FaqCategory(
        ...
        items = listOf(
            FaqItem(
                question = "为什么部分学习任务会被跳过？",
                answer = "只有能够确认请求语义和服务端结果的流程才会执行。" +
                    "文档、阅读、音频等不能证明实际学习行为的自动完成接口会被跳过，" +
                    "登录页、验证码、访问限制或未知响应也会立即停止当前批次。"
            ),
            FaqItem(
                question = "遇到学习通访问限制怎么办？",
                answer = "应用会在首个 403、429、验证码或限制页面出现后停止相关请求，并按服务端 Retry-After 冷却。" +
                    "请在学习通官方客户端确认账号状态或完成验证；冷却期间不要连续刷新。"
            ),
            FaqItem(
