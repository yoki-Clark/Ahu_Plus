...
import com.ahu_plus.data.model.AiCommentPrompts
...
class SessionManager(private val appDataStore: AppDataStore) : JwcNoticeCache {
    ...
    suspend fun init(): String? {
        ...
        if (!oldIdentity.isNullOrBlank() && newIdentitiesJson.isNullOrBlank()) {
            ...
            val school = runCatching {
                ...
                com.ahu_plus.data.remote.market.MarketApi.schoolFromIdentity(oldIdentity)
    ...
    /** 用户是否加入内测计划。默认 false。 */
    fun isBetaEnabled(): Boolean = cachedBetaEnabled

    suspend fun setBetaEnabled(enabled: Boolean) {

        cachedBetaEnabled = enabled

        appDataStore.dataStore.edit { it[BETA_ENABLED_KEY] = enabled }
    ...
    suspend fun saveCxNotopenAction(v: String) {
        cachedCxNotopenAction = v
        appDataStore.dataStore.edit { it[CX_NOTOPEN_ACTION_KEY] = v }
    }
    ...
    fun getCustomSignLocations(): List<com.ahu_plus.data.model.CustomSignLocation> =
        if (cachedCustomSignLocations.isBlank()) emptyList()
        else runCatching {
            gson.fromJson(
                cachedCustomSignLocations,
                Array<com.ahu_plus.data.model.CustomSignLocation>::class.java,
            ).toList()
        }.getOrDefault(emptyList())

    /** 覆盖保存自定义签到位置列表 */
    suspend fun saveCustomSignLocations(list: List<com.ahu_plus.data.model.CustomSignLocation>) {
