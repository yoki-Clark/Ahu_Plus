...
class CacheCleanupRepository(
    ...
    private val groupKeys: Map<String, List<Preferences.Key<String>>> = mapOf(
        ...
        GROUP_ACADEMIC to listOf(
            com.ahu_plus.data.local.SessionManager.SCHEDULE_JSON_KEY,
            com.ahu_plus.data.local.SessionManager.GRADES_JSON_KEY,
            com.ahu_plus.data.local.SessionManager.GPA_METADATA_JSON_KEY,
            com.ahu_plus.data.local.SessionManager.EXAMS_JSON_KEY,
            com.ahu_plus.data.local.SessionManager.TRAINING_PLAN_JSON_KEY,
            com.ahu_plus.data.local.SessionManager.EMPTY_CLASSROOM_JSON_KEY,
        ),
        // 学生信息(一张表)
        GROUP_STUDENT_INFO to listOf(
            com.ahu_plus.data.local.SessionManager.STUDENT_INFO_KEY,
        ),
        // 财务 / 考勤 / 一卡通账单
        GROUP_FINANCE_ATTENDANCE to listOf(
            com.ahu_plus.data.local.SessionManager.FINANCE_JSON_KEY,
            com.ahu_plus.data.local.SessionManager.ATTENDANCE_JSON_KEY,
            com.ahu_plus.data.local.SessionManager.KQCARD_ATTENDANCE_JSON_KEY,
            com.ahu_plus.data.local.SessionManager.BILLS_JSON_KEY,
        ),
        // 校园服务:评教 / 作业 / 课表备注 / 自定义任务 / 浴室电话 / 电费配置 / 支付码缓存
        GROUP_CAMPUS_SERVICES to listOf(
            com.ahu_plus.data.local.SessionManager.ASSESSMENT_JSON_KEY,
            com.ahu_plus.data.local.SessionManager.RECORD_INDEX_JSON_KEY,
            com.ahu_plus.data.local.SessionManager.HOMEWORK_JSON_KEY,
            com.ahu_plus.data.local.SessionManager.USER_TASKS_JSON_KEY,
            com.ahu_plus.data.local.SessionManager.USER_SCHEDULE_JSON_KEY,
            com.ahu_plus.data.local.SessionManager.ADWMH_QR_PAYLOAD_KEY,
            com.ahu_plus.data.local.SessionManager.ADWMH_QR_SERVER_TEXT_KEY,
        ),
        // 超星:业务缓存(登录态/凭据在 AUTH_DATA,不进 ALL_CLEARABLE_KEYS,不被此清理触及)
        GROUP_CHAOXING to listOf(
            com.ahu_plus.data.local.SessionManager.CX_COURSES_JSON_KEY,
            com.ahu_plus.data.local.SessionManager.CX_COURSES_PROGRESS_JSON_KEY,
            com.ahu_plus.data.local.SessionManager.CX_HOMEWORK_JSON_KEY,
            com.ahu_plus.data.local.SessionManager.CX_HOMEWORK_DETAIL_JSON_KEY,
            com.ahu_plus.data.local.SessionManager.CX_MESSAGES_JSON_KEY,
            com.ahu_plus.data.local.SessionManager.CX_TASK_LOG_KEY,
            com.ahu_plus.data.local.SessionManager.CX_TIKU_CACHE_KEY,
        ),
        // 公开数据:排考预测 / 天气 / 开发者公告
        GROUP_PUBLIC_DATA to listOf(
            com.ahu_plus.data.local.SessionManager.EXAM_PREDICTIONS_JSON_KEY,
            com.ahu_plus.data.local.SessionManager.WEATHER_JSON_KEY,
            com.ahu_plus.data.local.SessionManager.ANNOUNCEMENTS_JSON_KEY,
        ),
        // 应用记录:最近使用应用 + 已忽略公告 id
        GROUP_APP_RECORDS to listOf(
            com.ahu_plus.data.local.SessionManager.RECENT_APPS_KEY,
            com.ahu_plus.data.local.SessionManager.DISMISSED_ANNOUNCEMENT_IDS_KEY,
        ),
