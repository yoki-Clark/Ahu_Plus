        ...
        sections = listOf(
            ...
            LegalSection(
                ...
                paragraphs = listOf(
                    ...
                    "智慧安大验证码图片仅从 adwmh.ahu.edu.cn 获取，识别和展示均在用户设备本机完成（设备端机器学习模型推理或用户手动输入），不发送至任何外部 OCR 网站或识别服务。用户配置外部 AI、题库或通知地址前，应用会展示目标服务信息；相关内容只在用户主动使用时发送。",
                ),
            ),
            LegalSection(
                title = "五、设备权限",
    ...
    private val thirdPartyServices = LegalDocument(
        ...
        sections = listOf(
            LegalSection("安徽大学校园系统", bullets = listOf(
                ...
                "ycard.ahu.edu.cn、adwmh.ahu.edu.cn、kqcard.ahu.edu.cn：一卡通、支付码、余额、账单和考勤；可能处理会话及对应业务数据。智慧安大验证码仅在设备端识别或供手动输入。",
